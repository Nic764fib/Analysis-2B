...
import {nameCaption,recognitionHint,renderTheoremNames} from './theory-name-ui.js';
...
function recall(){
 ...
 const proof=t&&['mean','orthogonal'].includes(t.id);
 ...
 <div class="two-col"><label class="small">Modul<select id="recall-module"><option value="all" ${recallModule==='all'?'selected':''}>Alle Module</option><option value="klausur" ${recallModule==='klausur'?'selected':''}>Klausursätze lernen</option>${modules.map(m=>`<option value="${m.id}" ${recallModule===m.id?'selected':''}>${m.title}</option>`).join('')}</select></label>
 ...
 ${recallModule==='klausur'?`<p class="small muted">${Object.keys(examRecallSources).length} ausgewählte Theorie- und Beweiskarten aus Skript, Übungsblättern und Altklausur. ★ markiert die vier Schwerpunkte: Heine–Borel, Umkehrsatz, impliziter Satz und Taylor. Die vier Karten aus Blatt 1–2 bleiben zur Absicherung enthalten.</p><details class="recall-help"><summary>Alle ${Object.keys(examRecallSources).length} Klausurkarten ansehen</summary><ol class="recall-overview">${allTheory.filter(isExamRecall).map(c=>`<li><a href="#/${c.module}/theorie/${c.id}">${esc(c.title)}</a>${isExamFocus(c)?' <strong>★ Schwerpunkt</strong>':''}</li>`).join('')}</ol></details>`:''}
 ...
 ${proof?'<p class="small proof-reminder">Aussage und Beweis lernen · Altklausur 2025</p>':''}
 <p class="small muted">Formuliere ${t.kind.startsWith('Definition')?'die Definition':'die vollständige Aussage'} einschließlich aller Voraussetzungen.${proof?' Versuche anschließend auch den Beweis.':''}</p>
 ...
 ${proof?`<p class="small"><a href="#/${t.module}/aufgaben/${t.id==='mean'?'mean-proof':'laplace-proof'}">Vollständigen Beweis ansehen →</a></p>`:''}
...
function render(){const [id,tab,entry]=route();clearTimer();if(!id)home();else if(id==='abfragen')recall();else if(id==='satznamen')renderTheoremNames({shell,modules,entries:allTheory,quiz:tab==='abfragen'});else if(id==='klausur')exam();else if(id==='blaetter')worksheetPage(tab,entry);else if(id==='quellen')sourcePage();else{const m=modules.find(m=>m.id===id);if(m)modulePage(m,tab,entry);else shell('<h1>Modul nicht gefunden</h1><p><a href="#/">Zum Lernplan</a></p>','');}}
